...
export async function POST(request: NextRequest) {
  ...
  const upstream = await backendFetch('/auth/verify-otp', { method: 'POST', body });
  ...
  if (session.user.role !== 'ADMIN') return NextResponse.json({ message: 'این حساب دسترسی مدیریت ندارد' }, { status: 403 });
