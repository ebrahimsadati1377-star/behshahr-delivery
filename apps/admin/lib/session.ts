...
export const ACCESS_COOKIE = 'bd_admin_access';
export const REFRESH_COOKIE = 'bd_admin_refresh';
...
export function setSessionCookies(response: NextResponse, session: SessionPayload) {
  ...
  response.cookies.set(ACCESS_COOKIE, session.accessToken, {
    httpOnly: true, sameSite: 'lax', secure, path: '/', maxAge: session.accessTokenExpiresInSeconds,
  });
  response.cookies.set(REFRESH_COOKIE, session.refreshToken, {
    httpOnly: true, sameSite: 'lax', secure, path: '/', maxAge: session.refreshTokenExpiresInSeconds,
  });
}
